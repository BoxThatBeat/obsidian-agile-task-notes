  ...
  public async update(settings: AgileTaskNotesSettings): Promise<void> {
    ...
    try {
      if (settings.jiraSettings.mode == 'sprints') {
        const sprintsResponse = await requestUrl({
          method: 'GET',
          headers: headers,
        ...
        issueResponseList.forEach((issueResponse: RequestUrlResponse) => {
          issueResponse.json.issues.forEach((issue: any) => {
            if (
              !settings.jiraSettings.excludeBacklog ||
        ...
        if (settings.createKanban) {
          ...
          let columnIds = boardConfigResponse.json.columnConfig.columns.map((column: any) => column.name);
  ...
  public setupSettings(
    ...
    new Setting(container)
      ...
      .addDropdown((dropdown) => {
        ...
        dropdown.setValue(plugin.settings.jiraSettings.authmode).onChange(async (value) => {
    ...
    new Setting(container)
      ...
      .addDropdown((dropdown) => {
        ...
        dropdown.setValue(plugin.settings.jiraSettings.mode).onChange(async (value) => {
